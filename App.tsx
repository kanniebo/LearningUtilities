...
const App = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col text-slate-200 bg-[#0a0f1d] selection:bg-indigo-500/30">
      ...
      <main className="flex-grow container mx-auto max-w-7xl px-4 py-12 sm:px-8">
        ...
        {filteredGames.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center bg-slate-900/30 rounded-3xl border border-slate-800/50 backdrop-blur-sm">
            ...
            <h3 className="text-2xl font-bold text-white">Oops! Nothing here...</h3>
            <p className="text-slate-400 mt-2 text-lg">Try searching for something else!</p>
            <button 
              onClick={() => setSearchQuery('')}
              className="mt-8 px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-full transition-all hover:scale-105 active:scale-95"
            >
              Clear Search
            </button>
          </div>
      ...
      <footer className="bg-slate-900/50 backdrop-blur-md border-t border-slate-800/50 py-12 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-6">
          <div className="flex items-center gap-2 grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all cursor-default">
            ...
            <span className="text-white text-sm font-bold tracking-widest">LEARNING UTILITIES</span>
          </div>
          <div className="text-slate-500 text-[10px] sm:text-xs max-w-2xl text-center leading-relaxed font-medium space-y-2 uppercase tracking-tight">
            <p>
              Licensed under the Global Educational Open Access Agreement. All embedded software modules are provided 
              as-is for research and logical stimulation purposes. 
            </p>
            <p>
              Third-party content remains the intellectual property of original copyright holders. 
              Users must adhere to local network usage policies and digital literacy guidelines.
            </p>
          </div>
        </div>
      </footer>
